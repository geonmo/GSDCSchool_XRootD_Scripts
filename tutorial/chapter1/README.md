...
## 실습 개요
이번 실습을 통해 클라이언트측으로부터 바로 접속이 가능하고 도메인이 등록되어 있는 4개의 서버를 이용하여 XRootD 스토리지를 구축하는 방법을 익힙니다. 구축된 XRootD 스토리지는 클라이언트로부터 모든 노드가 개방되어 있기 때문에 여러 파일을 전송할 경우 노드의 개수만큼 네트워크를 사용할 수 있습니다.
