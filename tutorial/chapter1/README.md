...
이번 실습을 통해 4개의 서버를 이용하여 가장 간단한 방식의 XRootD 스토리지를 구축하는 방법을 익힙니다. 
   * 여기서 완전 개방형이란 다음과 같은 특징을 의미합니다. (공식용어 아니니 그냥 참고 바랍니다.)
      * XRootD 클라이언트(사용자)는 리다이렉트 서버 뿐만 아니라 각 디스크 서버들에 직접 접근이 가능해야 합니다. 
         * 예) ping
      * XRootD 클라이언트(사용자)는 리다이렉트 서버 뿐만 아니라 디스크 노드들의 호스트 이름(DNS 혹은 hosts 파일)를 알고 있어야 합니다.
